...
export default function Skills({ skills }: Skills_Props) {
	return (
		<div className="container flex flex-wrap items-center justify-center space-x-[.4rem]">
			{skills.map((skill) => (
				<div key={uuidv4()} className="no-select flex items-center justify-center space-x-[.4rem] px-[.8rem] py-[.5rem] mb-[.4rem] rounded-lg bg-gray-900 group transition-colors duration-300 ease-in-out hover:bg-gray-50">
					<Image className="max-h-5" src={skill.url} width={20} height={20} alt={skill.name} />
