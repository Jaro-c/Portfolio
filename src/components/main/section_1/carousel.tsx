			...
			<div className="relative h-[4rem] md:h-[8rem]">
				{text.map((item, index) => (
					<div key={uuidv4()} className={`absolute w-full h-full flex justify-center items-center opacity-0 ${index === currentIndex ? style["slide-in"] : index === prevIndex ? style["slide-out"] : ""}`}>
						<h2 className={`text-center leading-none text-[2rem] md:text-[4rem] xl:text-[6rem] font-bold bg-gradient-to-b from-indigo-400 to-cyan-400 bg-clip-text text-transparent py-[1rem]`}>{item}</h2>
					</div>
				))}
			</div>
		</div>
